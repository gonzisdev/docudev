...
import { callbackHandler, isCallbackSet } from './ws-server-callback'
...
const GC_TIMEOUT = parseInt(process.env.GC_TIMEOUT || '60000')
...
export const documentTimers = new Map()
...
const closeConn = (doc, conn) => {
  if (doc.conns.has(conn)) {
    ...
    if (doc.conns.size === 0) {
      console.log(
        `Document ${doc.name} without connections. Starting cleaning in (${
          GC_TIMEOUT / 1000
        }s)`
      )

      if (documentTimers.has(doc.name)) {
        clearTimeout(documentTimers.get(doc.name))
      }

      const timer = setTimeout(() => {
        doc.destroy()
        docs.delete(doc.name)
        documentTimers.delete(doc.name)
        console.log(`Document ${doc.name} removed from memory`)
      }, GC_TIMEOUT)

      documentTimers.set(doc.name, timer)
    }
  }
  conn.close()
}
